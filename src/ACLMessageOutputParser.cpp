...
//std::cout<<"retrieve\t"<<(*s).size()<<std::endl;
...
//std::cout<<"create it";
            ...
            for (it; it != (*s).end(); it++)
//                retstr.append(1,char(0x00)).append(bitParseParam(*it));
...
{
//std::cout<<(getBitBinWord((*p).getName()) + getBitBinExpression((*p).getValue(),'s'))<<std::endl;           
...
std::string ACLMessageOutputParser::getBitAID(AgentAID* aid, int depth)
{
            ...
            (( (*(*aid).getResolvers()).empty() == true)? "" : getBitAIDColl((*aid).getResolvers(),depth-1)) +
